...
def crosstalk_ItoQUV(data_demod, verbose=False, npoints=2000):
    ...
    if verbose:
        ...
        plt.text(mx - 0.9 * PLT_RNG * sx, min(my) - 1.4 * PLT_RNG * statistics.mean(sy),
                 'Cross-talk from I to Q: slope = {: {width}.{prec}f} ; off-set = {: {width}.{prec}f} '.format(cQ[0],
                                                                                                               cQ[1],
                                                                                                               width=8,
                                                                                                               prec=4),
        ...
        plt.text(mx - 0.9 * PLT_RNG * sx, min(my) - 1.55 * PLT_RNG * statistics.mean(sy),
                 'Cross-talk from I to U: slope = {: {width}.{prec}f} ; off-set = {: {width}.{prec}f} '.format(cU[0],
                                                                                                               cU[1],
                                                                                                               width=8,
                                                                                                               prec=4),
                 style='italic', bbox={'facecolor': 'blue', 'alpha': 0.1, 'pad': 1}, fontsize=15)
        plt.text(mx - 0.9 * PLT_RNG * sx, min(my) - 1.7 * PLT_RNG * statistics.mean(sy),
                 'Cross-talk from I to V: slope = {: {width}.{prec}f} ; off-set = {: {width}.{prec}f} '.format(cV[0],
                                                                                                               cV[1],
                                                                                                               width=8,
                                                                                                               prec=4),
                 style='italic', bbox={'facecolor': 'green', 'alpha': 0.1, 'pad': 1}, fontsize=15)
        plt.show()

    print('Cross-talk from I to Q: slope = {: {width}.{prec}f} ; off-set = {: {width}.{prec}f} '.format(cQ[0], cQ[1],
                                                                                                        width=8,
                                                                                                        prec=4))
    print('Cross-talk from I to U: slope = {: {width}.{prec}f} ; off-set = {: {width}.{prec}f} '.format(cU[0], cU[1],
                                                                                                        width=8,
                                                                                                        prec=4))
    print('Cross-talk from I to V: slope = {: {width}.{prec}f} ; off-set = {: {width}.{prec}f} '.format(cV[0], cV[1],
                                                                                                        width=8,
                                                                                                        prec=4))
...
def crosstalk_ItoQUV2d(data_demod, size=4):
    ...
    patchy = np.zeros((npatches, nwave, npol, size, size), dtype=float)
    ...
    for i in range(nwave):
        ...
        for j in range(npol):
            print('doing ', i, j)
            patchy[:, i, j, :, :] = image.extract_patches_2d(data_demod[i, j, :, :],
                                                             (size, size))  # ,max_patches=max_patches)
...
def phi_correct_fringes(data, header, option, fringe_threshold=0, verbose=False):
    '''
    ...
    if option == 'auto':
        ...
        level_theshold = [1.5, 1.5, 1.9]
        # level_theshold = [3.5,3.5,3.5]
        plt.ion()
        ...
        for i in range(zd // 4):
            for j in np.arange(1, 4):
                ...
                # power2d = gaussian_filter(power2d, sigma=(2, 2))
                ...
                imc_stat = imc[wsize - rad_max:wsize + rad_max + 1, wsize + rad_min:wsize + rad_max + 1]
                minimum = np.min(imc_stat)
                maximum = np.max(imc_stat)
                mean = np.median(imc_stat)
                rms = np.std(imc_stat)
                ...
                if len(idx[0]) > 0:
                    ...
                    for idx_i in range(len(idx[0])):
                        # if (imc[idx[0][idx_i],idx[1][idx_i]]) > level_theshold[j-1]*mean:
                        if (imc[idx[0][idx_i], idx[1][idx_i]]) > level_theshold[j - 1] * mean:
                            if (np.abs(np.sqrt((idx[0][idx_i] - sm[0] // 2) ** 2 + (
                                    idx[1][idx_i] - sm[1] // 2) ** 2)) > rad_min) and np.abs(
                                    np.sqrt((idx[0][idx_i] - sm[0] // 2) ** 2 + (
                                            idx[1][idx_i] - sm[1] // 2) ** 2)) < rad_max:
                                plt.plot(idx[1][idx_i], idx[0][idx_i], "og", markersize=3)
                                subm = imc[idx[0][idx_i] - win_halfw:idx[0][idx_i] + win_halfw + 1,
                                       idx[1][idx_i] - win_halfw:idx[1][idx_i] + win_halfw + 1]
                                if np.max(subm < 0):
                                    subm = 1 - subm
                                ...
                                f_gauss = 1 - np.exp(-(
                                            (x - xcoor) ** 2 / (2 * (width_x * 3) ** 2) + (y - ycoor) ** 2 / (
                                                2 * (width_y * 3) ** 2)))
                                # plib.show_four_row(subm,f_gauss,subm - f_gauss,imc)
                                F[idx[0][idx_i] + (h // 2 - wsize + 2) - win_halfw: idx[0][idx_i] + (
                                            h // 2 - wsize + 2) + win_halfw + 1,
                                idx[1][idx_i] + (w // 2 - wsize + 2) - win_halfw: idx[1][idx_i] + (
                                            w // 2 - wsize + 2) + win_halfw + 1] *= NFREQ_LEVEL  # f_gauss #win 1e-6#
                                power2d[idx[0][idx_i] + (h // 2 - wsize + 2) - win_halfw: idx[0][idx_i] + (
                                            h // 2 - wsize + 2) + win_halfw + 1,
                                idx[1][idx_i] + (w // 2 - wsize + 2) - win_halfw: idx[1][idx_i] + (
                                            w // 2 - wsize + 2) + win_halfw + 1] *= f_gauss  # win 1e-6#
                                print(freq_x[i, j - 1, loop], freq_y[i, j - 1, loop])
                                print(i, j, level_theshold[j - 1] * mean, 3. * level_theshold[j - 1] * mean, rms,
                                      3 * rms, imc[idx[0][idx_i], idx[1][idx_i]] - minimum, freq_x[i, j - 1, loop],
                                      freq_y[i, j - 1, loop])
                                loop += 1
        ...
        freq_x = np.zeros((3, 16))  # Frecuencias Maximo de 10 ventanas
        freq_y = np.zeros((3, 16))
        rad_min = int(10 * xd // 2048)  # for 2048 so we correct for smaller image size
        rad_max = int(35 * xd // 2048)  # for 2048 so we correct for smaller image size
        wsize = int(50 * xd // 2048)
        wbin = 1
        win_halfw = 1  # for 0
        ...
        print('*********', fringe_threshold, rad_min, rad_max, wsize, wbin, win_halfw, verbose)
        # if verbose:
        #     plt.ion()
        FF = np.zeros((zd, 3, yd, xd)).astype(complex)
        for i in range(zd // 4):
            for j in np.arange(1, 4):
                FF[i, j - 1, :, :] = np.fft.fftshift(np.fft.fft2(data[i, j, :, :]))
        for j in np.arange(1, 4):
            printc('Looking for fringes in Stokes ' + STOKESPAR[j], color=bcolors.WARNING)
            # Calculate power spectrum of the first (continuum) wavelength
            power2d = np.log10(np.abs((FF[0, j - 1, :, :] * np.conj(FF[0, j - 1, :, :])).astype(np.float)))
            # Calculate power spectrum of the first (continuum) wavelength
            # FFF = np.mean(FF,axis=0)
            # power2d = np.log10( np.abs( (FFF[j-1,:,:]*np.conj(FFF[j-1,:,:])).astype(np.float) ) )

            # Alternativelly, we could apply a gaussian filter to smooth the power
            # power2d = gaussian_filter(power2d, sigma=(1, 1))

            # Create power image as a square around a central area of _wsize radious
            im = power2d[yd // 2 - wsize:yd // 2 + wsize, xd // 2 - wsize:xd // 2 + wsize]
            # remove the edges (for the derivative)
            imc = im[2:-2, 2:-2]
            # do statistics of the power (centered in a small region)
            imc_stat = imc[wsize - rad_max:wsize + rad_max + 1, wsize + rad_min:wsize + rad_max + 1]
            px_stat = bin_annulus(imc.shape, rad_max, rad_max - rad_min + 1, full=False)
            if verbose:
                plt.imshow(px_stat)
                plt.show()

            imc_stat = imc[px_stat == True]
            minimum = np.min(imc_stat)
            maximum = np.max(imc_stat)
            mean = np.median(imc_stat)
            rms = np.std(imc_stat)
            print(minimum, maximum, mean, rms)
            stack = (
                    (im[2:-2, 2:-2] > shift(im, [-2, -2])[2:-2, 2:-2])
                    * (im[2:-2, 2:-2] > shift(im, [-2, -1])[2:-2, 2:-2])
                    * (im[2:-2, 2:-2] > shift(im, [-2, 0])[2:-2, 2:-2])
            ...
            if len(idx[0]) > 0:
                ...
                for idx_i in range(len(idx[0])):
                    ...
                    if px_stat[idx[0][idx_i], idx[1][idx_i]]:
                        # if (imc[idx[0][idx_i],idx[1][idx_i]]) > fringe_threshold*mean:
                        if (imc[idx[0][idx_i], idx[1][idx_i]]) > (mean + fringe_threshold * rms):
                            if verbose:
                                plt.plot(idx[1][idx_i], idx[0][idx_i], "og", markersize=3)
                            subm = imc[idx[0][idx_i] - win_halfw:idx[0][idx_i] + win_halfw + 1,
                                   idx[1][idx_i] - win_halfw:idx[1][idx_i] + win_halfw + 1]
                            if np.max(subm < 0):
                                subm = 1 - subm
                            height, xcoor, ycoor, width_x, width_y = moments(subm)
                            freq_x[j - 1, loop] = (idx[0][idx_i] - wsize + 2) / xd
                            freq_y[j - 1, loop] = (idx[1][idx_i] - wsize + 2) / yd
                            f_gauss = 1 - np.exp(-((x - xcoor) ** 2 / (2 * (width_x * 3) ** 2) + (y - ycoor) ** 2 / (
                                    2 * (width_y * 3) ** 2)))
                            print('Fringe threshold ', mean + fringe_threshold * rms, ' f_gauss ', height, xcoor, ycoor,
                                  width_x, width_y)
                            for i in range(zd // 4):
                                FF[i, j - 1, idx[0][idx_i] + (yd // 2 - wsize + 2) - win_halfw: idx[0][idx_i] + (
                                        yd // 2 - wsize + 2) + win_halfw + 1,
                                idx[1][idx_i] + (xd // 2 - wsize + 2) - win_halfw: idx[1][idx_i] + (
                                            xd // 2 - wsize + 2) + win_halfw + 1] *= NFREQ_LEVEL  # 1e-6
                                # FF[i,j-1, idx[0][idx_i] + (yd//2 - wsize + 2) - win_halfw : idx[0][idx_i]  + (yd//2 - wsize + 2) + win_halfw + 1 , idx[1][idx_i]  + (xd//2 - wsize + 2) - win_halfw : idx[1][idx_i]  + (xd//2 - wsize + 2) + win_halfw + 1].real *= f_gauss#1e-6
                                # FF[i,j-1, idx[0][idx_i] + (yd//2 - wsize + 2) - win_halfw : idx[0][idx_i]  + (yd//2 - wsize + 2) + win_halfw + 1 , idx[1][idx_i]  + (xd//2 - wsize + 2) - win_halfw : idx[1][idx_i]  + (xd//2 - wsize + 2) + win_halfw + 1].imag *= f_gauss*j#1e-6
                            printc('Detected in Stokes ', j, ' freq(x,y): ', freq_x[j - 1, loop], freq_y[j - 1, loop],
                                   loop)
                            loop += 1
                            if loop >= 16:
                                raise Exception("MANY FREQUENCIES IN FRINGE DETECTION. SOMETHING IS GOING WRONG")

                for i in range(zd // 4):
                    data[i, j, :, :] = np.fft.ifft2(np.fft.fftshift(FF[i, j - 1, :, :])).astype(np.float)
            if verbose:
                plt.colorbar()
                plt.show()  # block=True)
            if verbose:
        ...
        #  py = round(fy * sy)
        for j in np.arange(1, 4):
            print(j - 1, freq_y[j - 1, :], freq_x[j - 1, :])
    ...
    # -----------------
    # FRINGING - Need to include Ajusta senos contras!!!!
    # -----------------
    elif option == 'manual':
            ...
            for i in range(zd // 4):
                ...
                for l in range(len(px_x)):
                    ...
                    if px_x[l] != 0:  # add if condition for data with 1536 size
                        ...
                        index = np.where(ps[px_y[l]:px_y[l] + boxy[l] + 1, px_x[l]:px_x[l] + boxx[l] + 1] > 0)
                        if np.any(index):
                            row = index[0]
                            col = index[1]
                            xl = col + px_x[l]
                        ...
                        index = -1
                        # index = np.where(ps[px_y[l]:px_y[l]+boxy[l]+1,px_x[l]:px_x[l]+boxx[l]+1] < mean_ps - 0.5*sdev_ps)
                        index = np.where(ps[px_y[l]:px_y[l] + boxy[l] + 1, px_x[l]:px_x[l] + boxx[l] + 1] < np.Inf)
                        if np.any(index):
                            row = index[0]
                            col = index[1]
                            xl = col + px_x[l]
        ...
        for k in range(len(px_x_Q)):
            ...
            mask_QUV[0, px_y_Q[k] - win_halfw:px_y_Q[k] + win_halfw + 1,
            px_x_Q[k] - win_halfw:px_x_Q[k] + win_halfw + 1] *= (1 - maski)
        for k in range(len(px_x_U)):
            mask_QUV[1, px_y_U[k] - win_halfw:px_y_U[k] + win_halfw + 1,
            px_x_U[k] - win_halfw:px_x_U[k] + win_halfw + 1] *= (1 - maski)
        for k in range(len(px_x_V)):
            mask_QUV[2, px_y_V[k] - win_halfw:px_y_V[k] + win_halfw + 1,
            px_x_V[k] - win_halfw:px_x_V[k] + win_halfw + 1] *= (1 - maski)
        # if verbose:
...
def generate_level2(data, wave_axis, rte_mode, output_dir, ref_wavelenth=6173.3354, milos_executable=None, options=None,
                    center_line=True):
    '''
